...
from chat2map.common.sync_vector_env import SyncVectorEnv
...
class VectorEnvCustom:
    """Custom vectorized environment from v0.1.6 (master/stable or the one after 0.1.6)which creates multiple processes
    ...
    def __init__(
        ...
        """
        Create a custom vector environment
...
def construct_envs(
    ...
    :param config: configs that contain num_processes as well as information necessary to create individual environments.
    :param env_class: class type of the envs to be created
    :param workers_ignore_signals: flag saying if workers should ignore signals or not
    :param is_train: flag saying if environment will be used for training or eval
    ...
    if not workers_ignore_signals:
        ...
        envs = env_launcher(
            ...
            env_fn_args=tuple(
                tuple(zip(configs, env_classes, range(num_processes),
                    [is_train for i in range(num_processes)]))
            ),
        )
    else:
        assert is_train
        envs = VectorEnvCustom(
            ...
            env_fn_args=tuple(
                tuple(zip(configs, env_classes, [is_train for i in range(num_processes)]))
            ),
        )
    return envs
...
def make_env_fn(
    ...
    :param config: root exp config that has core env config node as well as env-specific config node.
    :param env_class: class type of the env to be created.
    :param rank: rank of env to be created (for seeding).
    :param is_train: flag saying if environment will be used for training or eval
    :return: env object created according to specification.
    """
    ...
    elif (config.EPS_SCENES_N_IDS != []) and (dataset_split[:5] != "train"):
...
def make_env_fn_custom(
    ...
    """
    creates an env of type env_class with specified config and rank when workers_ignore_signals is set to True.
    This is to be passed in as an argument when creating VectorEnv.
    :param config:
    :param env_class: root exp config that has core env config node as well as env-specific config node.
    :param is_train: class type of the env to be created.
    :return: env object created according to specification.
    """
