...
class BaselineRegistry(Registry):
    ...
    def register_trainer(cls, to_register=None, *, name: Optional[str] = None):
        """
        Register a RL training algorithm to registry with key 'name'.
        :param name: Key with which the trainer will be registered. If None will use the name of the class.
        :return:
        """

        from chat2map.common.base_trainer import BaseTrainer
    ...
    def register_env(cls, to_register=None, *, name: Optional[str] = None):
        """
        Register a environment to registry with key 'name'
            currently only support subclass of RLEnv.
        :param name: Key with which the env will be registered. If None will use the name of the class.
        :return:
        """
